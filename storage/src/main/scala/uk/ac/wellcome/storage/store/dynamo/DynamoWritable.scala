...
trait DynamoHashWritable[HashKey, V, T] extends DynamoWritable[Version[HashKey, V], DynamoHashEntry[HashKey, V, T], T] {
  implicit protected val formatV: DynamoFormat[V]
  assert(formatV != null)

  override protected def createEntry(id: Version[HashKey, V], t: T): DynamoHashEntry[HashKey, V, T] =
    DynamoHashEntry(id.id, id.version, t)

  override protected def tableGiven(id: Version[HashKey, V]): ConditionalOperation[DynamoHashEntry[HashKey, V, T], _] =
    table.given(
...
trait DynamoHashRangeWritable[HashKey, RangeKey, T] extends DynamoWritable[Version[HashKey, RangeKey], DynamoHashRangeEntry[HashKey, RangeKey, T], T] {
  implicit protected val formatRangeKey: DynamoFormat[RangeKey]
  assert(formatRangeKey != null)

  override protected def createEntry(id: Version[HashKey, RangeKey], t: T): DynamoHashRangeEntry[HashKey, RangeKey, T] =
    DynamoHashRangeEntry(id.id, id.version, t)

  override protected def tableGiven(id: Version[HashKey, RangeKey]): ConditionalOperation[DynamoHashRangeEntry[HashKey, RangeKey, T], _] =
    table.given(
