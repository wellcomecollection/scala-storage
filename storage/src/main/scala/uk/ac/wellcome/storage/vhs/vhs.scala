...
package object vhs {
  implicit val vhsEntryFormat: DynamoFormat[Entry[String, EmptyMetadata]] = new DynamoFormat[Entry[String, EmptyMetadata]] {
    override def read(av: AttributeValue): Either[DynamoReadError, Entry[String, EmptyMetadata]] =
      DynamoFormat[PlainEntry[String]].read(av).map {
        plainEntry =>
          Entry(
            id = plainEntry.id,
  ...
  implicit val updateExpressionGenerator: UpdateExpressionGenerator[Entry[String, EmptyMetadata]] =
