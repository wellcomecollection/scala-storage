...
import uk.ac.wellcome.storage.dynamo.{DynamoConfig, DynamoHashEntry, DynamoHashRangeEntry}
import uk.ac.wellcome.storage.maxima.Maxima
import uk.ac.wellcome.storage.maxima.dynamo.DynamoHashRangeMaxima
import uk.ac.wellcome.storage.store._


class DynamoHashRangeStore[HashKey, RangeKey, T](val client: AmazonDynamoDB, dynamoConfig: DynamoConfig)(
  implicit
  ...
  with DynamoHashRangeMaxima[HashKey, RangeKey, DynamoHashRangeEntry[HashKey, RangeKey, T]] {
  override protected val table = Table[DynamoHashRangeEntry[HashKey, RangeKey, T]](dynamoConfig.tableName)
}

class DynamoHashStore[HashKey, V, T](val client: AmazonDynamoDB, dynamoConfig: DynamoConfig)(
  implicit
  val formatHashKey: DynamoFormat[HashKey],
  ...
  override protected val table = Table[DynamoHashEntry[HashKey, V, T]](dynamoConfig.tableName)
}
