...
trait TypedStoreFixtures[Ident, T, StreamStoreImpl <: StreamStore[Ident, InputStreamWithLengthAndMetadata], StreamStoreContext] extends StreamStoreFixtures[Ident, StreamStoreImpl, StreamStoreContext] {
  ...
  def withTypedStore[R](streamStore: StreamStoreImpl, initialEntries: Map[Ident, TypedStoreEntry[T]])(testWith: TestWith[TypedStore[Ident, T], R])(implicit codec: Codec[T]): R

  def withTypedStoreImpl[R](storeContext: StreamStoreContext, initialEntries: Map[Ident, TypedStoreEntry[T]])(testWith: TestWith[TypedStore[Ident, T], R])(implicit codec: Codec[T]): R =
