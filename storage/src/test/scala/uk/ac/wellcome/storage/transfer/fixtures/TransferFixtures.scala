...
trait TransferFixtures[Ident, T, StoreImpl <: Store[Ident, T], TransferImpl <: Transfer[Ident], StoreContext] {
  def createT: T
  ...
  def withTransfer[R](testWith: TestWith[TransferImpl, R])(implicit context: StoreContext): R
}
