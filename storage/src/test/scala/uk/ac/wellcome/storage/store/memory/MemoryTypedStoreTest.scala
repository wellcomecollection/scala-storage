...
class MemoryTypedStoreTest extends TypedStoreTestCases[String, Record, String, MemoryStreamStore[String], MemoryTypedStore[String, Record], MemoryStore[String, MemoryStreamStoreEntry]] with MemoryTypedStoreFixtures[String, Record] with MetadataGenerators with RecordGenerators with StringNamespaceFixtures {
  ...
  override def withBrokenStreamStore[R](testWith: TestWith[MemoryStreamStore[String], R]): R = {
    val brokenMemoryStore = new MemoryStore[String, MemoryStreamStoreEntry](initialEntries = Map.empty) {
      override def get(id: String): Either[ReadError, Identified[String, MemoryStreamStoreEntry]] = Left(
        StoreReadError(new Throwable("get: BOOM!"))
      )

      override def put(id: String)(t: MemoryStreamStoreEntry): Either[WriteError, Identified[String, MemoryStreamStoreEntry]] = Left(
        StoreWriteError(
  ...
  override def withSingleValueStreamStore[R](rawStream: InputStream)(testWith: TestWith[MemoryStreamStore[String], R]): R = {
    val memoryStore = new MemoryStore[String, MemoryStreamStoreEntry](initialEntries = Map.empty)
